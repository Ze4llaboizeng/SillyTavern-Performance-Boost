...
export class VirtualScroll {
    /** @param {object} cfg  — from extension_settings.virtualScroll */
    constructor(cfg = {}) {
        this.cfg        = cfg;
    ...
    onChatChanged() {
        // Remove hidden-state from all messages of the old chat, then re-observe new ones
        this._clearAll();
        if (this.cfg.aggressiveMode) {
    ...
    onMessageAdded() {
        ...
        // Observe any message that isn't yet observed
    ...
    _setupIntersectionObserver() {
        // rootMargin: allow 2× viewport above/below before hiding
        const margin = `${window.innerHeight * 2}px 0px ${window.innerHeight * 2}px 0px`;
